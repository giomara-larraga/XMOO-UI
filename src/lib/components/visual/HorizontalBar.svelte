...
<script lang="ts">
	...
	import { colorPalette, referencePointStyle } from '$lib/components/visual/constants';
	import { getChartModel, roundToDecimal } from '$lib/components/visual/helperFunctions';
	...
	$: if (previousValue != null) {
		updatePreviousLine((previousValue = roundToDecimal(previousValue, decimalPrecision)));
	}
	...
	function addHorizontalBar(option: echarts.EChartOption) {
						...
						if (this.x > left && this.x < right) {
							if (Math.abs(this.x - params.offsetX) > 2) {
								selectedValue = roundToDecimal(
									chart.convertFromPixel({ seriesIndex: 0 }, [this.x, this.x])[0],
									decimalPrecision
								);
							} else {
								selectedValue = roundToDecimal(
								...
								axisPointer: {
									label: {
										formatter: () => {
											return (
												'Drag value: ' +
												roundToDecimal(selectedValue ? selectedValue : 0, decimalPrecision)
											);
										}
									}
								}
		...
		chart.getZr().on('click', function (params) {
			...
			if (targetParentName !== 'interactiveButtons' && params.target.id.toString() !== 'prevLine') {
	...
	function getLineComponent(chart: echarts.EChartsType, lineId: string) {
		let graphicComponent = chart.getOption().graphic as echarts.GraphicComponentOption;
		// Get the component
		// Type check
	...
	function showTooltip(params: {
		...
		switch (idToChek) {
			case 'prevLine':
				tooltipHelpText = 'Click this button to set the aspiration value to the previous value';
				break;
			case 'drag':
