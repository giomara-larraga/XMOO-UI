<!-- 
 ...
 -->
<script lang="ts">
	import Input from '$lib/components/visual/BasicInput.svelte';
...
<div class="container {aspect}">
	<div class="firstPart" style="--justify: {barName ? 'space-between' : 'flex-end'}">
		{#if barName}
			<span>{barName}</span>
		{/if}

		<Input bind:value={selectedValue} labelName="Aspiration level" onChange={moveToRange} />
		<!-- <div>
